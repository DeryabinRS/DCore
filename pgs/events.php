...
if($alias) {
    ...
    if ($new) {
        ...
        if($dateS){
            ...
            $event_date_content = '<div class="mb-2"><i class="fa fa-calendar-check-o"></i> '.$LANGJSON['news']['event_date'][$LANGUAGE].': ' . $event_date . ' '.$event_time.'</div>';
        }
    ...
    $PAGINATION = funcPagination('news', '*', get_param('sheet', 0 ,'int'),25,7,'type = 2 AND visible = 1', 'dateS DESC');
    $_CONTENT .= '<div class="container mt-4 mb-5">';
    $_CONTENT .= '<div class="row"><div class="col-12"><div class="mb-3"><a href="'.SITE_URL.'">'.$LANGJSON['page']['home'][$LANGUAGE].'</a> | <a href="'.SITE_URL.'/news">'.$_PAGENAME.'</a></div></div></div>';
    ...
    foreach ($PAGINATION['table'] as $row) {
        ...
        $_CONTENT .= '<div class="news_box mt-3">
                        '.$event_day.'
                            <a href="'.$new_url.'" style="display: inline-block">
                                <div class="news_box__img"><img src="'.$new_img.'" class="img-fluid"></div>
                            </a>
                                <div class="news_type mt-3">'.$type_new['name'.$LANGSQL].'</div>
                                <div class="news_info mb-2">'.date("d.m.Y", $row['date_pub']).'</div>
                        <a href="'.$new_url.'" style="display: inline-block"><div class="news_title mb-4">'.$row['name'.$LANGSQL].'</div></a>
                        <p class="mt-2"><a href="'.$new_url.'" class="btn btn-custom">'.$LANGJSON['news']['read_more'][$LANGUAGE].'</a></p>
                    </div>';
        $_CONTENT .= '</div>';
    }
    $_CONTENT .= '</div></div>';
